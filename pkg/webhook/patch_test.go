...
func TestPatchSparkPod_DNS(t *testing.T) {

	dnsVal := "5"
	dnsConfig := &corev1.PodDNSConfig{
		Nameservers: []string{"1.1.1.1", "2.2.2.2"},
		Searches:    []string{"a.domain", "otherdomain"},
		Options: []corev1.PodDNSConfigOption{
			corev1.PodDNSConfigOption{Name: "ndots", Value: &dnsVal},
		},
	}
	...
	assert.NotEmpty(t, modifiedDriverPod.Spec.DNSConfig)
	assert.Equal(t, dnsConfig, modifiedDriverPod.Spec.DNSConfig)
	...
	assert.NotEmpty(t, modifiedExecutorPod.Spec.DNSConfig)
	assert.Equal(t, dnsConfig, modifiedExecutorPod.Spec.DNSConfig)
}
